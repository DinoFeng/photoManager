...
log4js.getLogger = (category) => {
  ...
  l.timeEnd = (name, level, desc) => {
    ...
    // l[log](`${desc || name}: durationMs = ${(diff[0] * NS_PER_SEC + diff[1]) / MS_PER_SEC} ms`)
    l[log](`${desc || name} durationMs:${durationMs}`)
    // console.timeEnd(name)
  }
  l.profile = (name, level, desc) => {
    const start = profile[name]
    start ? l.timeEnd(name, level, desc) : l.time(name)
    // if (start) {
